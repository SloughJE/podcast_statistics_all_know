...
<h1 style="font-size: 80px; margin-top: 200px;">Beware the Average</h1>

<hr style="margin-top: 2em; margin-bottom: 2em;">

--
<h2 style="font-size: 60px;">It Can Be Mean</h2>
...
### Local bar weath distribution: 50 Patrons + Bill Gates
...
# ─ your sample sizes (100, 1 000, 10 000, 100 000; extend as needed) ─
ns     <- 10^(2:5)
bill   <- 150e9

# ─ draw the largest once ─────────────────────────────────────────────────
wealth_master <- rlnorm(max(ns), meanlog = 11, sdlog = 0.4)

# ─ compute mean/median for each n + Bill ─────────────────────────────────
df <- tibble(n = ns) %>%
  mutate(
    sample     = map(n, ~ wealth_master[1:.x]),
    all_vals   = map(sample, ~ c(.x, bill))
  ) %>%
  rowwise() %>%
  mutate(
...
```{r mean_med_n_data, include=FALSE, echo=FALSE, results='asis'}

slide_text <- purrr::map_chr(seq_len(nrow(df)), function(i) {
  paste0(
    "---\n\n",
    "## Number of people + Bill Gates: ", format(df$n[i], big.mark = ","), "\n\n",
    "<p><span style='color:", mean_col, "'><strong>Mean:</strong></span> $",
      format(round(df$mean_val[i]), big.mark = ","), "</p>\n\n",
    "<p><span style='color:", median_col, "'><strong>Median:</strong></span> $",
      format(round(df$median_val[i]), big.mark = ","), "</p>\n"
  )
})
...
## Number of people + Bill Gates: 10,000

<p><span style='color:#C04E3D'><strong>Mean:</strong></span> $1,662,250</p>

<p><span style='color:#BFA06D'><strong>Median:</strong></span> $57,983</p>
