...
<h1 style="font-size: 80px; margin-top: 200px;">Shapes of Change</h1>
...
```{r linear_non, echo = FALSE, message = FALSE, warning = FALSE}
library(dplyr)
library(tidyr)
library(stringr)

curve_data <- data.frame(x = seq(0, 5, 0.1)) %>%
  mutate(
    linear = 4 * x,
    exponential = 2^x - 1,               # shift so at x=0, y=0
    quadratic = x^2,
    logarithmic = (log(x) - log(0.1)) * 3.5
  )

long_data <- curve_data %>%
  pivot_longer(-x, names_to = "type", values_to = "y")

# Define shorthand mapping
shorthand <- c(
