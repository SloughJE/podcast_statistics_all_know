...
<li><b>Test:</b> Does A cause B? Does melatonin increase sleep duration?</li>
<br>
<li><b>Randomization:</b> Assign by chance to treatment or control</li>
<br>
<li><b>Control:</b> Keep other factors balanced across groups (via randomization)</li>
<br>
<li><b>Treatment:</b> Apply the intervention</li>
<br>
<li><b>Comparison:</b> Check for outcome differences between groups</li>
</ul>
</div>
]
...
```{r, causality, fig.width=4.5, fig.height=4, echo=FALSE, message=FALSE, warning=FALSE}

#fig.cap="Compare outcomes between randomized groups to estimate causal effect"
library(ggplot2)
library(dplyr)
...
# Create plot with group-based color
...
> using randomization to balance everything else,
...
<span style="font-size: 20px;">**Bias** is the systematic difference between the expected result of an estimator and the true underlying value it's trying to measure.</span>
<br>

--

<span style="font-size: 20px;">It's the **consistent gap** between what your method tends to estimate and the actual truth — not just random error, but a built-in shift.</span>
<br>

--

<span style="font-size: 20px;">Bias means you’re consistently off target because of how you measure, think, or sample — and you don’t realize it.</span>
...
# Generate data
unbiased <- rnorm(n, mean = true_value, sd = 5)
biased <- rnorm(n, mean = true_value + 10, sd = 5)

df <- data.frame(
  estimate = c(unbiased, biased),
  type = rep(c("Unbiased", "Biased"), each = n)
)
...
# Plot
p_bias = ggplot(summary_df, aes(x = type, y = mean, color = type)) +
  geom_point(size = 4) +
...
ggplotly(p_bias, height = 300, width = 400) %>%
  layout(
    annotations = list(
      list(
...
ggplotly(p_bias, height = 400, width = 500) %>%
  layout(
    annotations = list(
      list(
        x = 0.24, y = summary_df$mean[summary_df$type == "Biased"]-0.15,
        xref = "paper", yref = "y",
      ...
      list(
        x = 0.75, y = summary_df$mean[summary_df$type == "Unbiased"]-0.08,
        xref = "paper", yref = "y",
...
<div style="display: flex; justify-content: space-between; font-size: 18px;">
  <ul style="list-style-type: disc; padding-right: 40px;">
    <li><b>Selection bias</b>: Non-representative sample</li>
    <li><b>Omitted variable bias</b>: Missing important predictors</li>
    <li><b>Measurement bias</b>: Data collection errors</li>
    <li><b>Survivorship bias</b>: Ignoring those who dropped out</li>
  </ul>
  <ul style="list-style-type: disc;">
    <li><b>Recall bias</b>: Faulty memories skewing data</li>
    <li><b>Observer bias</b>: Researcher expectations shape results</li>
...
<span style="font-size: 28px;">Our minds filter reality through prior beliefs and assumptions, or <b>bias</b>.</span> 
<br>

<span style="font-size: 28px;">Recognizing bias helps you understand: </span>

<ul style="font-size: 20px; line-height: 1.2;"> 
  <li>how it can shape attention and memory</li> 
  <li>why we favor evidence that fits what we already believe</li> 
  <li>how feedback loops can reinforce or shift our views over time</li>
  <li>how bias can escalate, growing from subtle differences in interpretation to extreme positions over time</li>
</ul>
--

<span style="font-size: 22px;">Without awareness, bias can change how we see the world, gradually strengthening and pushing us further from reality.</span>

--

<span style="font-size: 22px;">From time to time ask yourself:</span>

>Where am I on the Bias Escalation Curve?
